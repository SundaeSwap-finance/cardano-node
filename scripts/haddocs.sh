...
for row in $(
  cat dist-newstyle/cache/plan.json | \
    jq -r '
      ...
      | ."install-plan"[]
      | select(.style == "local")
      | { "dist-dir": ."dist-dir", "pkg-name": ."pkg-name", "component-name": ."component-name"}
      | @base64'
); do
    ...
    if [ -d "${dist_dir}" ]; then
      for doc_index in $(find "${dist_dir}" -name doc-index.html); do
        ...
        if [ -f "${OUTPUT_DIR}/${pkg_name}/${component_name}/${package}.haddock" ]; then
          echo "--read-interface=${package},${OUTPUT_DIR}/${pkg_name}/${component_name}/${package}.haddock"
        fi
      done
    fi
  done
